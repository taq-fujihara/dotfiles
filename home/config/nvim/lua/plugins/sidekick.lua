return {
  ...
  config = function(_, opts)
    ...
    local function attach_cmd(self)
      if self.herdr_terminal_id then return { cmd = { "herdr", "terminal", "attach", self.herdr_terminal_id } } end
    end

    function Herdr:init()
      self.is_running = function(s) return s.herdr_pane_id and herdr_json { "pane", "get", s.herdr_pane_id } ~= nil end
    end
    ...
    function Herdr:start()
      ...
      if pi and pi.result and pi.result.pane then self.herdr_terminal_id = pi.result.pane.terminal_id end
    ...
    function Herdr:send(text)
      if self.herdr_pane_id then vim.fn.system { "herdr", "pane", "send-text", self.herdr_pane_id, text } end
    end

    function Herdr:submit()
      if self.herdr_pane_id then vim.fn.system { "herdr", "pane", "send-keys", self.herdr_pane_id, "enter" } end
    end
    ...
    config.validate = function(key, t)
      if key == "cli.mux.backend" then t = vim.list_extend(vim.deepcopy(t), { "herdr" }) end
